...
section .data
	...
	ignore 			equ 0					; Codes for the different operations
	leftPar 		equ 1					
	rightPar 		equ 2
	powerTo 		equ 3
...
CheckIfSub:
		...
		cmp byte [eax - 1], asciiPowerTo
		je OperandsLoopCheck
		mov byte [operationAt + ebx], subtraction
		jmp ConvertNumbers			
...
PowLoop:
		...
		cmp byte [operationAt + ecx], powerTo  			;Compares the operation to ^
...
FindFirstPowNumber:
		...
		cmp byte [operationAt + edx - 1], ignore 	;Check if the number at EDX-1 should be ignored
...
PowerTo:
		...
		pop ecx
		mov [numbers + eax], edx
		mov byte [operationAt + ecx], 0 	;Make the other number ignored by calculations
...
MulLoop:
		...
		cmp byte [operationAt + ecx], multiplication 	;Compares the operation to multiplication
...
FindFirstMulNumber:
		...
		cmp byte [operationAt + edx - 1], division 	;Check if the number at EDX-1 is a divisor
...
DivLoop:
		...
		cmp byte [operationAt + ecx], division 		 	;Compares the operation to division
...
AddLoop:
		...
		cmp byte [operationAt + ecx], addition 			;Compares the operation to addition
...
SubLoop:
		...
		cmp byte [operationAt + ecx], subtraction 		;Compares the operation to subtraction
